...
class Sm4
{
    ...
    public function encrypt($data, $type = 'sm4', $iv = '', $formatOut = 'hex')
    {
        ...
        switch ($type) {
            case 'sm4':
            case 'sm4-cbc':
                $data = $this->mystr_pad($data, $this->len); //需要补齐
                $ret = $this->enDataCbc($data, $iv);
                break;
            case 'sm4-ecb':
                $data = $this->mystr_pad($data, $this->len); //需要补齐
                $ret = $this->enDataEcb($data);
                break;
            case 'sm4-ctr':
                $ret = $this->enDataCtr($data, $iv);
                break;
            case 'sm4-ofb':
                $ret = $this->enDataOfb($data, $iv);
                break;
            case 'sm4-cfb':
                $ret = $this->enDataCfb($data, $iv);
                break;
            default:
    ...
    public function decrypt($data, $type = 'sm4', $iv = '', $formatInput = 'hex')
    {
        ...
        switch ($type) {
            case 'sm4':
            case 'sm4-cbc':
                $ret = $this->deDataCbc($data, $iv);
                $ret =  $this->mystr_unpad($ret);
                break;
            case 'sm4-ecb':
                $ret = $this->deDataEcb($data);
                $ret =  $this->mystr_unpad($ret);
                break;
            case 'sm4-ctr':
                $ret = $this->deDataCtr($data, $iv);
                break;
            case 'sm4-ofb':
                $ret = $this->deDataOfb($data, $iv);
                break;
            case 'sm4-cfb':
                $ret = $this->deDataCfb($data, $iv);
                break;
            default:
                throw new \Exception('bad type');
        }
